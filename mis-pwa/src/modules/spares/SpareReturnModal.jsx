...
const SpareReturnModal = ({ spare, onClose, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await client.post('/spares/return', payload);

      alert('Spare returned successfully');
      onClose();
      if (onSaved) await onSaved();
    } catch (err) {
      console.error('Return spare error:', err);
      alert(err?.response?.data?.message || 'Failed to return spare');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={`Return Spare - ${spare?.part_code || ''}`} onClose={onClose}>
...
export default SpareReturnModal;
